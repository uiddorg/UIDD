...
bool ContextualCheckCoinSpend(const CoinSpend& spend, CBlockIndex* pindex, const uint256& txid)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
            return error("%s : zUIDD with serial %s is already in the block %d\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zUIDD spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckCoinSpend(spend, chainActive.Tip(), txid))
                    return state.Invalid(error("%s: zUIDD spend in tx %s failed to pass context checks", __func__, txid.GetHex()));
            }
        } else {
            LOCK(pool.cs);
	...
	if (nHeight > 0)
	{
		...
		else if (nHeight <= Params().LAST_POW_BLOCK())
		{
			...
			else
			{
				...
				nSubsidy = 988456990 * COIN; // Satoshis. (1684806479500000 (Number of satoshis in circulation on BTC block 507850) - (101 * COIN)) / (Params().LAST_POW_BLOCK() - 202);
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Uidd
...
{
    RenameThread("uidd-scriptch");
    scriptcheckqueue.Thread();
}

/*void RecalculateZUIDDMinted()
{
...
void RecalculateZUIDDSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zUIDD supply
...
bool RecalculateUIDDSupply(int nHeightStart)
{
...
{
    // Uidd: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZUIDDSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
	...
	LogPrint("masternode", "//Track zUIDD money supply in the bl\n");
    //Track zUIDD money supply in the block index
    if (!UpdateZUIDDSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zUIDD supply for block=%s height=%d", __func__,
    ...
    /*LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zUIDDSpent: %s\n",
	...
	LogPrint("masternode", "//Record zUIDD serials\n");
    //Record zUIDD serials
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert UIDD to zUIDD
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zUIDD spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zUIDD serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    //if (nMints || nSpends)  LogPrintf("%s : block contains %d zUIDD mints and %d zUIDD spends\n", __func__, nMints, nSpends);
