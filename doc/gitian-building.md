...
Clone the git repositories for uidd and gitian and then checkout the uidd version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the uidd repository
...
To build Uidd (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the uidd repository.
...
```bash
    ...
    From https://github.com/uidd-crypto/uidd
...
For example:
```bash
URL=https://github.com/crowning-/uidd.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/uidd-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/uidd-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/uidd-build.assert
```
...
[uidd/gitian.sigs](https://github.com/uidd-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
